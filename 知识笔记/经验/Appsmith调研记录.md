...
#### 连接数据源
1. 直接连本地的mongo![[Pasted image 20250321173027.png]]
2. 添加一个查询，我们从theme这个collections中直接插所有数据![[Pasted image 20250321173109.png]]
3. 配置到表格中，可以直接显示出数据![[Pasted image 20250321173128.png]]
4. 可以发现appsmith在数据源这块的设计确实很强大，但是前提是后端集成或者支持这么干
...
>[!Error] 内存占用
>不可忽视的内容占用，chrome渲染进程达到了将近1g，只是设计器，且设计器中内容只有一个表格，一个按钮，一个自定义组件，如果再嵌入到我们自己的项目中。。。![[Pasted image 20250321173647.png]]
